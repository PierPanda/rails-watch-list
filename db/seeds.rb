...
pages = (1..10).to_a
pages.each do |page|
  url = URI("https://api.themoviedb.org/3/movie/popular?language=en-US&page=#{page}")
  ...
  request["Authorization"] = 'Bearer eyJhbGciOiJIUzI1NiJ9.eyJhdWQiOiJjYzljNzRlMmVjMTljYmI1OTg5NzY1ZTE5ZmQ3OGU0NCIsIm5iZiI6MTcyMzQ4MjM3NS4yMjgwNjksInN1YiI6IjY2Yjc4YTFmODNhNTVmYjk4YzAxN2YxZCIsInNjb3BlcyI6WyJhcGlfcmVhZCJdLCJ2ZXJzaW9uIjoxfQ.3JHtFubFKbhzN-NCNeWi-CJcSJqzy4Xnn0DNggB67ck'

  response = http.request(request)
  movies = JSON.parse(response.read_body).dig("results")

  movies.each do |movie|
    movie = Movie.new(title: movie['title'], overview: movie['overview'], poster_url: movie['poster_path'], rating: movie['vote_average'])
    movie.save
  end
end
